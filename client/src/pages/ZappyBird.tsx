...
import { Sound } from '../zappy_bird/runtime/sound';
import { BoltSDK } from '@boltpay/bolt-js';
import './ZappyBird.css';

const VolumeMuteIcon = () => (
  <svg class="zappy-volume-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
export default function ZappyBird() {
  ...
  useEffect(() => {
    ...
    const gamePromise = startZappyBird(canvasRef.current!);

    return () => {
      gamePromise.then((cleanup) => cleanup?.());
    };
  }, []);
